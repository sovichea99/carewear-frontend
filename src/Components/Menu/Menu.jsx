...
const Menu = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await api.get('/products'); // Use the same API as in Products
  ...
  if (loading) return <p className='flex justify-center items-center text-center'>Loading...</p>;
      ...
      <div className="container lg:w-full pt-1 pb-2">
        ...
        <div className="text-center mt-6">
          <Link
            ...
            className="text-pink-500 text-md font-semibold hover:text-pink-700"
          >
            --{'>'} show more products
          </Link>
        </div>
